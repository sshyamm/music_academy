...
class Profile {
    ...
    public function checkUser() {
        $checkSql = "SELECT user_id, user_name, user_password FROM $this->table WHERE BINARY user_name = :user_name";
        ...
        if ($this->user_password === $row['user_password']) {
            return array('success' => true, 'user_id' => $row['user_id'], 'user_name' => $row['user_name']);
        } else {
